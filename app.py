...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://azzouzhamza:@localhost:5432/udacity_pr'
db = SQLAlchemy(app)
...
class Venue(db.Model):
    ...
    def __repr__(self):
      return  f'<Venue : {self.id}, name :{self.mame}, city : {self.state}, address : {self.address} >'
...
def venues():
  ...
  #     num_upcoming_shows should be aggregated based on number of upcoming shows per venue. (done above)
  #extraire les donnes stocker dans la base de donnees:
  #main porpose of this route is to display data the same way presented on dummy data 
  ...
  for venue in venues:
    ...
    cities_states.sort(key=itemgetter(1,0))     # Sorts on second column first (state), then by city usinf sort() with itemgetter key
    # itemgetter() can be used to sort the list based on the value of the given key.
    now = datetime.now()    
    for c_s in cities_states: # boucle for pour passer sur toutes les localisations
...
@app.route('/venues/create', methods=['GET']) #getting the form to fill
...
def create_venue_submission():
  ...
  try:
    ...
    seeking_talent = True if 'seeking_talent' in request.form else False  #looking for talent, checkbox
    seeking_description = request.form.get['seeking_description']
    #match every user entry with the correspondant element in our Venue model
    ...
    db.session.add(venue) #fill the database
    db.session.commit()
  except: 
    error = True
    db.session.rollback()
  ...
  if error:
    flash('An error was occured. Venue  ' + request.form['name'] +' could not be listed')
  if not error:  
...
def delete_venue(venue_id):
  ...
  error = False
  try:
    venue = Venue.query.get(venue_id) # get venue id to proceed action on it
    db.session.delete(venue) #sqlalchemy delete query
    db.session.commit() #mark the change
  except:
    error = True
    db.session.rollback()
    print(sys.exc_info())
...
def search_artists():
  ...
  #the same goes here as like venue model : 
  ...
  # replace with real artist data from the artist table, using artist_id
   ...
   shows = Show.query.filter_by(artist_id=artist_id).all() #query the all the model and class by artis_id
   pastShows = []
   upcomingShows = []
   ...
   for show in shows:
      ...
      if date < datetime.now():
        venue_query = Venue.query.filter_by(id=show.venue_id)
        pastShows.append({ 
            "venue_id": venue_query.first().id,
            "venue_name": venue_query.first().name,
            "venue_image_link": venue_query.first().image_link,
            "start_time": show.start_time
        })
      #if date is in futur,append to the upcoming show   
      if date > datetime.now():
...
#edit route for every venue_id route from the DB model:
#let's get models data to edit :
@app.route('/venues/<int:venue_id>/edit', methods=['GET'])
def edit_venue(venue_id):
  form = VenueForm()
  venue = Venue.query.get(venue_id)
  # populating form by default when opening route :
  if venue: 
  ...
  #   "image_link": "https://images.unsplash.com/photo-1543900694-133f37abaaa5?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=400&q=60"
  # }
  # !!! populate form with values from venue with ID <venue_id> \ done , (testing)
  return render_template('forms/edit_venue.html', form=form, venue=venue)

@app.route('/venues/<int:venue_id>/edit', methods=['POST'])
def edit_venue_submission(venue_id):
  # TODO: take values from the form submitted, and update existing  --- View to controller ---
